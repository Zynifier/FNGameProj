...
void AWaterBrushManager::PostLoad()
{
	...
	if (!IsTemplate() && bNeedsForceUpdate)
	{
...
{
#if WITH_EDITOR
	if (GetOwningLandscape() != nullptr)
	{
		// This engine's ALandscape has no ComputeLandscapeLayerBrushInfo : the brush receives that data through Initialize instead.
		if (LandscapeRTRes.X > 0 && LandscapeRTRes.Y > 0)
		{
